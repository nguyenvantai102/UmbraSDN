...
class Profile:
    ...
    def get_node(self, node):
        ...
        logger.debug(f"Info in get_node function - node profile: {node_profile}")
    ...
    def get_link(self, link):
        ...
        logger.debug(f"Info in get_link function - link profile: {link_profile}")
    ...
    def look_for(self, _type, where):
        if where == "nodes":
            ...
            logger.debug(f"In loog_for function - itemset: {itemset}")
        elif where == "links":
        ...
        logger.debug(f"types: {types} and _type: {_type}")
        if types:
            (k, v) = types.pop()
...
class Topology(Graph):
    ...
    def add_node_profile(self, resources, profile=None, node_name=None):
        if profile or node_name:
            ...
            logger.info("Node profile is added")
        else:
    ...
    def add_link_profile(self, resources, profile):
        if profile:
            ...
            logger.info("Link profile not added")
        else:
    ...
    def build_environments(self):
        ...
        for sw in switches:
            ...
            if sw_env not in envs:
                logger.debug(f"{sw_env} is not in {envs} => Create envs")
                envs[sw_env] = {
                    "nodes": {},
                    "links": {},
                }
            ...
            for sw_neigh in self.graph.neighbors(sw):
                ...
                if sw_edge_type == "internal":
                    ...
                    logger.debug(f"Links add to {sw_env} with {sw_edge_type}")
    ...
    def build(self):
        ...
        for n, data in self.graph.nodes(data=True):
            ...
            logger.debug(f"Info resources {resources}")
            logger.debug(f"Info lifecycle {lifecycle}")
            node.update(resources)
            node.update(lifecycle)
            node_id = node.get("name")
            nodes[node_id] = node
            logger.debug(f"Info {nodes}")
