...
Environment
    ...
    def _parse_node_postgres_template(self, node):
        ...
        logger.info(f"Volume: {node_template.get('volumes', [])}")
        return node_kwargs
    ...
    def add_iroha_node(
        ...
        ack_profile = ack.get("profile", None)
        logger.info(f"Profile {ack_profile}")
    ...
    def add_network(self, net, envid=None):
        ...
        if envid in self._environments:
            if net not in self._networks:
                ...
                logger.info(f"Network added - name {net} already start")
            else:
